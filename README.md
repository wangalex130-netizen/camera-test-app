...
- **实时画面 (画面)**：用 AndroidX Media3 (ExoPlayer + RTSP) 直接拉流播放，可在手机上**实时查看摄像头画面**（已预填 `rtsp://admin:abc123456@192.168.1.60:554/11`，强制 TCP 传输更稳）。
- **延时摄影 (延时)**：先播放 RTSP 流，再按设定的**间隔（秒）**与**总张数**周期抓取视频帧，逐张存为 JPEG。Android 10+ 直接写入相册 `Pictures/CameraTestApp/`，旧系统写入应用专属目录；实时显示进度与最近保存路径。
